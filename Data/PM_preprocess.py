# data preprocessing for pathway modules
...
def process_and_save_omics_data(input_dir, omics_file_names, output_dir):
    ...
    # Load "pathway_module.csv"
    pathway_module = pd.read_csv(os.path.join(input_dir, "pathway_module.csv"), index_col=0).dropna(axis=1)
    pathway_module_columns = pathway_module.columns
    
    intersected_columns = set(pathway_module_columns)
    for omics_file in omics_file_names:
        ...
        intersected_columns &= set(omics_data.columns)
    
    intersected_columns = list(intersected_columns)

    # Only keep intersected columns for pathway_module
    pathway_module = pathway_module[intersected_columns]
    ...
    for omics_file in omics_file_names:
        ...
        # Only keep intersected columns for omics_data
        omics_data = omics_data[intersected_columns]
    ...
    # Save the pathway_module to a single CSV file
    pathway_module.to_csv(os.path.join(output_dir, "pathway_module.csv"), index_label="SAMPLE_ID")
    
    print(f"Processed pathway modules saved in {os.path.join(output_dir, 'pathway_module.csv')}")
...
input_dir = "./"
output_dir = input_dir + "PM_OmicsData/"
